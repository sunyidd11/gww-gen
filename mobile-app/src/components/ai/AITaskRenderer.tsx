...
import { getLocationTaskPresentation } from '../../aiTaskFlow';
import type {
  AITask,
  AppointmentData,
  DoctorOption,
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        ...
        {activeTask.type === 'location' && (() => {
          const presentation = getLocationTaskPresentation(activeTask.data as LocationData);
          const { location } = presentation;
          return (
            <div className="w-full space-y-5 sm:space-y-6">
              {presentation.showPresetMap ? (
                <div className="space-y-4 sm:space-y-5">
                  <div className="overflow-hidden rounded-3xl border border-green-100 bg-white shadow-sm">
                    <div className="flex items-center gap-2 border-b border-green-100 bg-green-50 px-4 py-3 text-sm font-bold text-green-700 sm:px-5 sm:text-base">
                      <Navigation size={18} />
                      <span>{location.routePreview?.title ?? '步行导航'}</span>
                    </div>
                    <div className={`${presentation.mapPanelClassName} relative bg-gradient-to-br from-[#E8F7EF] via-[#F5FBF7] to-[#EAF3FF] p-5 sm:p-6`}>
                      <div className="absolute inset-0 opacity-70">
                        <div className="absolute left-[14%] top-[18%] h-3 w-[68%] rounded-full bg-green-200/80" />
                        <div className="absolute left-[18%] top-[32%] h-[42%] w-3 rounded-full bg-green-200/70" />
                        <div className="absolute right-[16%] top-[26%] h-[48%] w-3 rounded-full bg-blue-200/70" />
                        <div className="absolute left-[24%] top-[58%] h-3 w-[46%] rounded-full bg-green-300/70" />
                      </div>
                      <div className="relative flex h-full flex-col justify-between">
                        <div className="flex items-start justify-between gap-3">
                          <div className="rounded-2xl bg-white/85 px-4 py-3 shadow-sm backdrop-blur-sm">
                            <div className="text-xs font-bold uppercase tracking-[0.18em] text-green-700/70">目的地</div>
                            <div className="mt-1 text-xl font-black text-green-900 sm:text-2xl">{location.title}</div>
                          </div>
                          {location.routePreview?.eta ? (
                            <div className="rounded-full bg-hospital-blue px-4 py-2 text-sm font-bold text-white shadow-lg">
                              {location.routePreview.eta}
                            </div>
                          ) : null}
                        </div>
                        <div className="flex items-end justify-between gap-3">
                          <div className="rounded-2xl bg-white/90 px-4 py-3 shadow-sm backdrop-blur-sm">
                            <div className="flex items-center gap-2 text-sm font-semibold text-green-700">
                              <MapPin size={16} /> 预设地图窗口
                            </div>
                            <div className="mt-1 text-xs text-gray-500 sm:text-sm">已按步行导航模式放大展示</div>
                          </div>
                          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-green-600 text-white shadow-xl sm:h-16 sm:w-16">
                            <MapPin size={28} />
                          </div>
                        </div>
                      </div>
                  ...
                  <div className={`rounded-2xl border border-gray-100 bg-gray-50 p-4 sm:p-5 ${presentation.infoPanelClassName}`}>
                    <div className="text-sm font-bold text-gray-500 sm:text-base">地点信息</div>
                    {location.fields.length > 0 ? (
                      <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3">
                        {location.fields.map((field) => (
                          <div key={`${field.label}-${field.value}`} className="rounded-xl bg-white px-3 py-2.5 shadow-sm">
                            <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-gray-400">{field.label}</div>
                            <div className="mt-1 text-sm font-semibold text-gray-800 sm:text-base">{field.value}</div>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <div className="text-sm text-gray-400">暂无地点信息</div>
                    )}
                  </div>
                </div>
              ) : (
                <div className="rounded-3xl border border-green-100 bg-green-50 p-6 text-center sm:p-8">
                  ...
                  <h3 className="mb-2 text-2xl font-black text-green-900 sm:text-3xl">{location.title}</h3>
                  {location.fields.length > 0 ? (
                    <div className="mx-auto mt-4 grid max-w-xl grid-cols-1 gap-3 text-left sm:grid-cols-2">
                      {location.fields.map((field) => (
                        <div key={`${field.label}-${field.value}`} className="rounded-2xl bg-white/70 px-4 py-3 shadow-sm">
                          <div className="text-xs font-bold uppercase tracking-[0.18em] text-green-700/70">{field.label}</div>
                          <div className="mt-1 text-base font-bold text-green-900 sm:text-lg">{field.value}</div>
                        </div>
                      ))}
                    </div>
                  ) : null}

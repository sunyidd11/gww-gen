...
export default function TaskNavigator(props: TaskNavigatorProps) {
  ...
  return (
    <div className="rounded-[28px] border border-[#ebe8fa] bg-[#F3F4FF] p-4 shadow-[0_12px_28px_rgba(108,81,233,0.08)]">
      <p className="text-[14px] font-semibold text-[#8e88b6]">{tr(props.lang, "相关任务推荐", "Related Tasks")}</p>
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {related.map((item) => (
          <Link
            key={item.slug}
            href={`/tasks/${item.slug}`}
            className="rounded-[20px] border border-[#ece9fb] bg-white px-3 py-3 text-center text-[15px] font-semibold text-[#3d3959] shadow-[0_8px_18px_rgba(61,57,89,0.12)] transition hover:bg-[#faf9ff]"
          >
